...
def udp_send_and_wait(dst_host, dst_port, payload: dict, timeout=0.3) -> Tuple[bool, Tuple[str,int], dict]:
    ...
    while True:
        ...
        if t == "DISCOVER":
            ...
            if decision == "ACCEPT":
                ...
                # wait for GAME_TCP from same addr
                while True:
                    s.settimeout(10.0)
                    try:
...
def run_host(username: str, bind_host: str, tcp_port: int, peer_udp: Tuple[str,int]):
    ...
    us.sendto(json.dumps({"type":"GAME_TCP","host":bind_host,"port":tcp_port}).encode("utf-8"), (uh, up))
    us.close()
...
def run_guest(username: str, host: str, port: int):
    ...
    if not msg or msg.get("type") != "WELCOME":
        ...
        return
    mark = msg.get("mark", "?")
    first = msg.get("first","X")
    print(f"[Game] You are 'O'. Host is 'X'. Rule: recycle-3. First: {first}")
    while True:
        msg = recv_json(s)
        if not msg:
